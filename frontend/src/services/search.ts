 ...
 * @returns {Promise<ISearchResult[]>} A promise that resolves to an array of response data.
 *
...
export async function getSearchResult(value: string): Promise<ISearchResult[]> {
  try {
    const endpoint: string = `/api/users?q=${value}`;
    const response: ISearchResult[] = await API.get(endpoint).then(
      (res) => res.data,
    );
